...
def validate_field(key, value):
    ...
    Returns True if the field is valid, False if it should be discarded.
    """
    ...
    # Reject if value is too short (likely OCR noise)
    if len(value) < 2:
        return False
    
    # Reject if value is ONLY numbers/symbols with no letters (likely OCR error)
    if not re.search(r'[a-zA-Z]', value):
        # Exception: allow pure numbers for specific fields
        if any(x in key.lower() for x in ['amount', 'total', 'price', 'postal', 'zip', 'phone', 'date', 'code', 'number', 'id']):
            # Should have at least some digits
            if re.search(r'\d', value):
                return True
        return False
    
    # Reject if value contains too many special characters (OCR noise)
    special_count = len(re.findall(r'[^a-zA-Z0-9\s\-.,;:()@#$%&₹€£¥]', value))
    if special_count > len(value) * 0.4:  # More than 40% special chars
        return False
    
    # Reject if value has excessive repeated characters (OCR error)
    if re.search(r'(.)\1{5,}', value):  # 6+ repeated chars
        return False
    
    # Reject if value looks like garbled text (too many consonants in a row)
    if re.search(r'[bcdfghjklmnpqrstvwxyz]{8,}', value, re.IGNORECASE):
        return False
    ...
    if 'postal' in key.lower() or 'zip' in key.lower():
        # Postal code should be mostly digits
        digits = len(re.findall(r'\d', value))
        if digits < len(value) * 0.4:  # Less than 40% digits
            return False
    
    if 'amount' in key.lower() or 'total' in key.lower() or 'price' in key.lower():
        # Should contain at least one digit
        if not re.search(r'\d', value):
            return False
    
    if 'date' in key.lower():
        # Date should have numbers and separators
        if not re.search(r'\d', value):
            return False
        # Should not be just a year
        if re.match(r'^\d{4}$', value):
            return False
    
    if 'address' in key.lower():
        # Address should not be just a number
...
<script>
  ...
  function renderResults(data) {
    ...
    console.log('DEBUG: Fields received:', Object.keys(data.fields || {}));
    ...
    for (const [k, v] of Object.entries(data.fields || {})) {
      ...
      if (isItem) {
        console.log('DEBUG: Found item:', k, '=', v);
    ...
    console.log('DEBUG: Items to display:', itemKeys.length);
    
    // Add items section if we have items
    if (itemKeys.length > 0) {
      console.log('DEBUG: Adding items section');
      ...
      itemKeys.forEach(item => {
        console.log('DEBUG: Adding item to grid:', item);
      ...
      console.log('DEBUG: No items found to display');
    }
...
def parse_amazon(t):
    ...
    # Don't apply universal detail extraction for Amazon - it adds too many false positives
    # result = extract_universal_details(t, result)
...
def parse_cbs(t):
    ...
    def clean_ocr_text(text):
        ...
        # Remove leading OCR garbage - symbols and corrupted text
        # Remove patterns like "=<) = —", "~—", "=<)", "— ", etc. (symbols with spaces/dashes)
        text = re.sub(r'^[~=<>\-\|()]+(\s+[~=<>\-\|()]+)*\s*', '', text)  # Remove symbol groups
        text = re.sub(r'^[\-—~]+\s*', '', text)  # Remove leading dashes/em-dashes
        text = re.sub(r'^(?:aula\s+y|Pang|Te|nen|wen|re\s+i|~\s+\d+|sMegmeme)\s+', '', text)  # Remove OCR garbage words
        text = re.sub(r'^[A-Za-z]{1,2}\s+(?=[A-Za-z])', '', text)  # Remove "EB ", "f ", "rr ", etc.
        text = re.sub(r'^[a-z]{1,3}\)\s+', '', text)  # Remove "ei) ", "ce) ", etc.
        text = re.sub(r'^[a-z]\s+[;:,\-]\s+', '', text)  # Remove "f ; ", "mi fe I "
        text = re.sub(r'^\([a-z]{1,2}\)\s+', '', text)  # Remove "(ei) ", "(ce) ", etc.
        ...
        text = re.sub(r'\s+Refunded\s+\d+\s*$', '', text, flags=re.IGNORECASE)
        
        # Fix common OCR misreadings
        text = re.sub(r'\baz\b', 'oz', text)  # "az" -> "oz"
        text = re.sub(r'\bmi\}', 'ml}', text)  # "mi}" -> "ml}"
        text = re.sub(r'\bmv\b', 'ml', text)  # "mv" -> "ml"
        text = re.sub(r'\bmi\b(?!\s*\()', 'ml', text)  # "mi" -> "ml" (but not in "mi)")
    ...
    def is_valid_product_name(name):
        ...
        if letters < len(name) * 0.25:  # At least 25% letters (lowered from 30%)
            return False
        
        # Reject obvious non-products
        if re.search(r'\b(?:final|item|price|qty|amount|rate|total|subtotal|tax|fee|charge|items found|order totals|items purchased|replacements|not charged|adjustments)\b', name, re.I):
            return False
    ...
    def extract_items_email_format(t):
        """
        Extract items from email format with multiple section types:
        - ADJUSTMENTS
        - REPLACEMENTS
        - ITEMS FOUND
        """
        ...
        # Split by section headers to get each section separately
        # First, find all section boundaries
        sections = {}
        
        for section_name in ['ADJUSTMENTS', 'REPLACEMENTS', 'ITEMS FOUND', 'ORDER TOTALS']:
            match = re.search(f'^{section_name}', t, re.MULTILINE)
            if match:
                sections[section_name] = match.start()
        
        # Extract each section
        section_names = ['ADJUSTMENTS', 'REPLACEMENTS', 'ITEMS FOUND']
        for section_name in section_names:
            ...
            for other_section in section_names + ['ORDER TOTALS']:
                if other_section != section_name and other_section in sections:
                    if sections[other_section] > start:
                        end = min(end, sections[other_section])
            
            section_text = t[start:end]
            items.extend(_extract_from_section(section_text, section_name))
    ...
    def _extract_from_section(section_text, section_name):
        ...
        while i < len(raw_lines):
            ...
            if re.match(r'^(ITEMS FOUND|ADJUSTMENTS|REPLACEMENTS|NOT CHARGED|Some of your items)', line, re.IGNORECASE):
                i += 1
                continue
            
            # Look for price pattern: "2x $44.75" or "1x $8.95" or "1 x $8.95"
            price_match = re.search(r'(\d+)\s*\*?\s*x\s*\$\s*([\d\.]+)', line)
            ...
            if not price_match and re.match(r'^\$\s*[\d\.]+\s*$', line):
                ...
                for j in range(i - 1, max(0, i - 4), -1):
                    ...
                    if any(skip in prev_line.upper() for skip in ['ITEMS FOUND', 'REAL CANADIAN', 'ORDER TOTALS', 'ADJUSTMENTS', 'REPLACEMENTS']):
                        continue
            ...
            if price_match:
                ...
                for j in range(i - 1, max(0, i - 5), -1):
                    ...
                    if any(skip in prev_line.upper() for skip in ['ITEMS FOUND', 'REAL CANADIAN', 'ORDER TOTALS', 'REPLACEMENTS', 'ADJUSTMENTS', 'NOT CHARGED', 'ORIGINAL PRICE']):
                        continue
                    
                    if re.search(r'^\d+\s*\*?\s*x\s*\$', prev_line):
                        continue
...
def preprocess_receipt_image(img: "Image.Image") -> "Image.Image":
    """Sharpen and boost contrast so tesseract reads receipts more accurately."""
...
def ocr_image_bytes(image_bytes: bytes) -> str:
    """Run pytesseract on raw image bytes and return extracted text."""
    img = Image.open(io.BytesIO(image_bytes))
    img = preprocess_receipt_image(img)
    # PSM 4 = single column of text (ideal for receipts)
    config = "--psm 4 --oem 3"
...
def parse_receipt_text(t: str) -> dict:
    """
    Parse OCR text from a shop / grocery / restaurant receipt into
    structured fields using regex. Enhanced to detect business types
    and extract relevant information for DMart, grocery, and restaurant bills.
    """
    result = {}
    ...
    # ── Store identity ──────────────────────────────────────────────────────
    # Find the first non-empty, non-numeric, non-symbol line as store name
    for line in lines[:5]:
        if len(line) >= 3 and re.search(r'[A-Za-z]{3,}', line) and not re.match(r'^[\d\W]+$', line):
            result["store_name"] = line
            break

    # ── Enhanced parsing based on business type ────────────────────────────
    if business_type == "grocery":
        # Extract grocery-specific information
        gst_reg = m(r'GST\s*(?:REG|REGISTRATION|#)\s*[:\-]?\s*([A-Z0-9]+)')
        if gst_reg:
            result["gst_registration"] = gst_reg
            
        # Store number for chain stores
        store_num = m(r'Store\s*(?:#|No\.?|Number)\s*[:\-]?\s*(\d+)')
        if store_num:
            result["store_number"] = store_num
            
        # Manager on duty
        manager = m(r'(?:Manager|MGR)\s*[:\-]?\s*([A-Z\s]+)')
        if manager:
            result["manager"] = manager
            
    elif business_type == "restaurant":
        # Extract restaurant-specific information
        table_num = m(r'Table\s*(?:#|No\.?)\s*[:\-]?\s*(\d+)')
        if table_num:
            result["table_number"] = table_num
            
        server = m(r'(?:Server|Served\s+by|Waiter|Waitress)\s*[:\-]?\s*([A-Z\s]+)')
        if server:
            result["server"] = server
            
        # Tip information
        tip = m(r'(?:Tip|Gratuity)\s*\$?\s*([\d,]+\.\d{2})')
        if tip:
            result["tip"] = tip
            
        # Number of guests
        guests = m(r'(?:Guests?|Party\s+Size|Covers?)\s*[:\-]?\s*(\d+)')
        if guests:
            result["number_of_guests"] = guests

    # ── Store location and contact info ─────────────────────────────────────
    for line in lines[1:8]:
        if re.search(r'\d+.*(?:Ave|St|Blvd|Rd|Dr|Way|SE|SW|NE|NW|Lane|Ln|Hwy)\b', line, re.I):
    ...
    # Prefer REF# lines, then generic receipt/transaction patterns
    ref = m(r'REF#\s*(\w+)')
    if ref:
    ...
    # TIME label takes priority; otherwise grab last HH:MM:SS in text
    time_ = m(r'\bTIME\s+(\d{1,2}:\d{2}(?::\d{2})?)')
    if not time_:
        all_times = re.findall(r'\b(\d{1,2}:\d{2}:\d{2})\b', t)
        if all_times:
            time_ = all_times[-1]   # last timestamp = transaction time
    if time_:
        result["time"] = time_
    ...
    # ── Enhanced loyalty program detection ──────────────────────────────────
    member = m(r'(?:Member|Loyalty|Scene\+?|Club|Air\s*Miles|PC\s*Points)\s*(?:card\s*)?(?:number|#|no\.?)?\s*[:\-]?\s*([\*x\d]{4,})')
    if member:
        result["member_card"] = member

    loyalty_pts = m(r'(?:Scene\+?|Points?|Rewards?|Air\s*Miles)\s+Balance\s*[:\-]?\s*(\d+)')
    if loyalty_pts:
        result["loyalty_points_balance"] = loyalty_pts
        
    # Points earned this transaction
    points_earned = m(r'(?:Earn|Earned|You\s+Earned)\s+(\d+)\s+(?:Scene\+?|Points?|Air\s*Miles)')
    if points_earned:
        result["points_earned"] = points_earned

    # ── Additional receipt information ──────────────────────────────────────
    # Terminal/POS information
    terminal = m(r'(?:Terminal|Term|POS)\s*[:\-]?\s*([A-Z0-9]+)')
    if terminal:
        result["terminal_id"] = terminal
        
    # Merchant information
    merchant = m(r'(?:Merchant|MERCH)\s*[:\-]?\s*([A-Z0-9]+)')
    if merchant:
        result["merchant_id"] = merchant
        
    # Batch number
    batch = m(r'(?:Batch|BATCH)\s*[:\-]?\s*([A-Z0-9]+)')
    if batch:
        result["batch_number"] = batch
        
    # Store hours or return policy
    hours = m(r'(?:Hours?|Open)\s*[:\-]?\s*([0-9AM-PM\s\-:]+)')
    if hours and len(hours) > 5:
        result["store_hours"] = hours
        
    # Return policy days
    return_policy = m(r'(?:Return|Exchange).*?(\d+)\s+days?')
    if return_policy:
        result["return_policy_days"] = return_policy

    # ── Line items ──────────────────────────────────────────────────────────
    # Words that appear on summary/payment lines — never product names
    SKIP = re.compile(
        r'^\s*(?:subtotal|sub total|total tax|total|tax|visa|mastercard|cash|debit|'
        r'change|tender|scene|member|balance|savings?|discount|hst|gst|pst|qst|'
        r'approved|receipt|transaction|ref|auth|terminal|merchant|number of items|'
        r'items?\s+purchased|card\s+vi|appl\.|aid\s+|rcpt|resp|date|time|'
        r'earn\s+\d|learn\s+more|scotiabank|\*\*\s*purchase|00\s*$|served\s+by|'
        r'store\s+name|store\s+address|phone|website|hours|return|policy|'
        r'you\s+saved|total\s+savings|points?\s+balance|points?\s+earned|'
        r'amount\s+tendered|business\s+type|cashier|merchant\s+id|terminal\s+id|'
        r'reference\s+id|transaction\s+id|payment\s+method|store\s+address|'
        r'freshco|sobeys|loblaws|metro|walmart|costco)$',  # Store names at end of line
        re.I
    )
    # Also skip lines that are entirely symbols / digits with no letters
    NOISE = re.compile(r'^[\d\W\s]+$')

    items = []

    # Strategy 1 — More flexible item detection with cent symbol support
    # Look for lines that have text followed by a price, being more permissive
    item_patterns = [
        # Pattern 1: "Item Name    $4.69 C" or "Item Name    4.69 C" or "Item Name $4.69 ¢"
        re.compile(r'^(.{3,60}?)\s+\$?([\d,]+\.\d{2})\s*[CT¢]?\s*$', re.MULTILINE),
        # Pattern 2: "Item Name $4.69" (no tax indicator)
        re.compile(r'^(.{3,60}?)\s+\$?([\d,]+\.\d{2})\s*$', re.MULTILINE),
        # Pattern 3: More flexible spacing with cent symbol
        re.compile(r'^(.{3,50})\s{2,}\$?([\d,]+\.\d{2})\s*[CT¢]?\s*$', re.MULTILINE),
        # Pattern 4: Handle items that might have extra characters
        re.compile(r'^(.{3,60}?)\s+\$?([\d,]+\.\d{2})\s*[CT¢]?\s*.*$', re.MULTILINE),
    ]
    
    for pattern in item_patterns:
        if items:  # If we already found items, don't try other patterns
            break
            
        for hit in pattern.finditer(t):
            raw_name = hit.group(1).strip()
            price = hit.group(2).replace(',', '')
            
            # Skip obvious non-items (more comprehensive)
            if SKIP.search(raw_name):
                continue
            if NOISE.match(raw_name):
                continue
                
            # Skip discount and savings lines
            if re.search(r'(?:discount|you\s+saved|total\s+savings|your\s+total\s+savings)', raw_name, re.I):
                continue
                
            # Must contain at least 2 letters
            if len(re.findall(r'[A-Za-z]', raw_name)) < 2:
                continue
                
            # Skip lines that look like addresses
            if re.search(r'\d+.*\b(SE|SW|NE|NW|Ave|St|Rd|Blvd)\b', raw_name, re.I):
                continue
                
            # Skip phone numbers
            if re.match(r'^\(?\d{3}\)?[\s\-\.]\d{3}[\s\-\.]', raw_name):
                continue
                
            # Skip GST registration numbers
            if re.match(r'^GST\s*#?\s*[A-Z0-9]+', raw_name, re.I):
                continue
                
            # Skip merchant/terminal IDs
            if re.match(r'^(?:MERCHANT|TERMINAL|CARD|NO\.|AUTH)', raw_name, re.I):
                continue
                
            # Skip very short names (likely not items)
            if len(raw_name.strip()) < 3:
                continue
                
            # Skip lines that are mostly numbers or symbols
            if len(re.findall(r'\d', raw_name)) > len(re.findall(r'[A-Za-z]', raw_name)):
                continue
                
            # Skip lines with too many asterisks (discount lines)
            if raw_name.count('*') > 3:
                continue
            
            items.append({"name": raw_name, "total_price": price})

    # Strategy 2 — Look for common grocery item patterns
    if not items:
        # Look for lines that contain common grocery terms
        grocery_terms = re.compile(r'\b(?:bread|milk|eggs|cheese|butter|yogurt|juice|'
                                 r'chicken|beef|pork|fish|apple|banana|orange|'
                                 r'carrot|lettuce|tomato|potato|onion|'
                                 r'cereal|pasta|rice|flour|sugar|salt|'
                                 r'bakery|produce|dairy|meat|frozen|'
                                 r'buns?|loaf|dozen|lb|lbs|kg|pack|bottle|can|jar)\b', re.I)
        
        for line in lines:
            if grocery_terms.search(line):
                # Try to extract price from this line
                price_match = re.search(r'\$?([\d,]+\.\d{2})', line)
                if price_match:
                    price = price_match.group(1)
                    # Remove the price from the name
                    name = re.sub(r'\s*\$?[\d,]+\.\d{2}\s*[CT]?\s*$', '', line).strip()
                    if len(name) > 2 and not SKIP.search(name):
                        items.append({"name": name, "total_price": price})

    # Strategy 3 — Look between store info and totals
    if not items:
        # Find the section between store header and totals
        subtotal_idx = -1
        store_end_idx = -1
        
        for i, line in enumerate(lines):
            if re.search(r'(?:served\s+by|cashier|member\s+card)', line, re.I):
                store_end_idx = i
            elif re.search(r'(?:subtotal|sub\s+total)', line, re.I):
                subtotal_idx = i
                break
        
        if store_end_idx > 0 and subtotal_idx > store_end_idx:
            # Look for items in this section
            for i in range(store_end_idx + 1, subtotal_idx):
                line = lines[i]
                # Look for lines with prices
                if re.search(r'\$?[\d,]+\.\d{2}', line) and not SKIP.search(line):
                    price_match = re.search(r'\$?([\d,]+\.\d{2})', line)
                    if price_match:
                        price = price_match.group(1)
                        name = re.sub(r'\s*\$?[\d,]+\.\d{2}\s*[CT]?\s*$', '', line).strip()
                        if len(name) > 2 and len(re.findall(r'[A-Za-z]', name)) >= 2:
                            items.append({"name": name, "total_price": price})

    # Detect "N @ $X.XX" quantity sub-lines and merge into preceding item
    qty_re = re.compile(r'^(\d+)\s*@\s*\$?([\d\.]+)', re.MULTILINE)
    merged = []
    i = 0
    while i < len(items):
        itm = items[i]
        if i + 1 < len(items):
            qh = qty_re.match(items[i + 1]["name"])
            if qh:
                itm["qty"] = qh.group(1)
                itm["unit_price"] = qh.group(2)
                i += 2
                merged.append(itm)
                continue
        merged.append(itm)
    ...
    for idx, itm in enumerate(merged, 1):
        label = itm["name"]
        if "qty" in itm:
            label += f" (x{itm['qty']} @ ${itm['unit_price']})"
        result[f"item_{idx:02d}"] = f"{label} → ${itm['total_price']}"
        
    # Add item count if we found items
    if merged:
        result["items_found"] = str(len(merged))

    # ── Enhanced totals parsing ─────────────────────────────────────────────
    # Try multiple patterns for subtotal
    subtotal = m(r'(?:Sub\s*[-]?Total|SUBTOTAL|SUB\s+TOTAL)\s*\$?\s*([\d,]+\.\d{2})')
    if subtotal:
        result["subtotal"] = subtotal

    # Enhanced tax parsing - handle multiple tax types
    total_tax = m(r'(?:Total\s+Tax|TOTAL\s+TAX|TAX\s+TOTAL|HST|H\.S\.T\.)\s*\$?\s*([\d,]+\.\d{2})')
    if total_tax:
        result["total_tax"] = total_tax

    gst = m(r'\b(?:GST|G\.S\.T\.)\b\s*\$?\s*([\d,]+\.\d{2})')
    if gst:
        result["gst"] = gst

    pst = m(r'\b(?:PST|P\.S\.T\.)\b\s*\$?\s*([\d,]+\.\d{2})')
    if pst:
        result["pst"] = pst
        
    # Quebec sales tax
    qst = m(r'\b(?:QST|Q\.S\.T\.)\b\s*\$?\s*([\d,]+\.\d{2})')
    if qst:
        result["qst"] = qst

    # Match standalone TOTAL line with enhanced patterns
    total = m(r'^(?:TOTAL|Total)\s*\$?\s*([\d,]+\.\d{2})', re.MULTILINE)
    if not total:
        total = m(r'(?:Grand\s+)?Total\s*\$?\s*([\d,]+\.\d{2})')
    if not total:
        # Handle "TOTAL TAX" followed by "TOTAL" pattern
        total = m(r'TOTAL\s+TAX.*?\n.*?TOTAL\s*\$?\s*([\d,]+\.\d{2})', re.DOTALL)
    if total:
        result["total"] = total
    ...
    # Enhanced card detection - handle various formats
    card = m(r'(?:XX+|NO\.|Card\s+VI)[\s\*X]*(\d{4})\b')
    if not card:
        card = m(r'CARD\s+VI\D*(\d{4})\b')
    if not card:
        # Handle "VISA 2804" format
...
@app.route("/ping")
def ping():
    return jsonify({"status": "ok", "receipt_parser": "parse_receipt_text", "version": "enhanced_with_items"})
...
def extract():
    ...
    try:
        ...
        parser = PARSERS.get(invoice_type, parse_generic)
        # Pass the temp file path to parsers that need direct PDF access (e.g. DHL)
        parse_dhl._current_pdf_path = tmp.name
        parse_amazon._current_pdf_path = tmp.name
        fields = {k: v for k, v in parser(raw_text).items() if v}
        
        # Debug: Count items
        items_count = len([k for k in fields.keys() if k.startswith('item_')])
        print(f"DEBUG: Invoice type: {invoice_type}, Parser: {parser.__name__}, Items found: {items_count}")
        ...
        return jsonify({
            ...
            "fields": fields,
            "raw_text_preview": raw_text[:600] + ("..." if len(raw_text) > 600 else ""),
            "debug_items_count": items_count,  # Debug field
        })

  ...
  function renderResults(data) {
    ...
    for (const [k, v] of Object.entries(data.fields || {})) {
      if (k.startsWith('item_')) {
        ...
        itemKeys.push({key: k, value: v});
      } else {
        otherFields.push({key: k, value: v});
      }
    }
    
    // Sort items in ascending order (item_01, item_02, item_03, etc.)
    itemKeys.sort((a, b) => {
      const numA = parseInt(a.key.replace('item_', ''));
      const numB = parseInt(b.key.replace('item_', ''));
      return numA - numB;
    });
    
    console.log('DEBUG: Items to display (sorted):', itemKeys.length);
    
    // Display non-item fields first
    for (const field of otherFields) {
      const label = field.key.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      const isMoney = moneyKeys.some(m => field.key.toLowerCase().includes(m));
      const isHighlight = highlightKeys.some(h => field.key.toLowerCase().includes(h));
      grid.innerHTML += `<div class="fkey">${label}</div><div class="fval${isMoney ? ' money' : ''}${isHighlight ? ' highlight' : ''}">${field.value || '—'}</div>`;
    }
    
    // Add items section if we have items (sorted in ascending order)
    if (itemKeys.length > 0) {
      console.log('DEBUG: Adding items section with', itemKeys.length, 'items');
      ...
      itemKeys.forEach((item, index) => {
        ...
        grid.innerHTML += `<div class="fkey">${label}</div><div class="fval" style="font-family: 'DM Mono', monospace; font-size: 12px;">${item.value}</div>`;
      });
    } else {
...
def parse_fedex(t):
    ...
    # ── Enhanced Sender Address Extraction ─────────────────────────────────
    # Extract complete sender address block
    sender_block = re.search(r'Sender\s*\n(.*?)(?=Recipient|Automation|Transportation)', t, re.DOTALL)
    if sender_block:
        sender_lines = [line.strip() for line in sender_block.group(1).split('\n') if line.strip()]
        
        # First line is usually the name/company
        if sender_lines:
            result["sender_name"] = sender_lines[0]
        
        # Extract sender address components
        sender_address_parts = []
        sender_city_state_zip = ""
        sender_country = ""
        
        for i, line in enumerate(sender_lines[1:], 1):
            # Check for city, state/province, postal code pattern
            ...
            if city_state_zip_match:
                ...
                sender_city_state_zip = line
                continue
            
            # Check for country (usually last line or after postal code)
            if line.upper() in ['CANADA', 'UNITED STATES', 'USA', 'US', 'CA']:
            ...
            if phone_match:
                result["sender_phone"] = phone_match.group(1)
                continue
        ...
        if "sender_name" in result:
            full_sender_address.append(result["sender_name"])
        if sender_address_parts:
            full_sender_address.extend(sender_address_parts)
        if sender_city_state_zip:
            full_sender_address.append(sender_city_state_zip)
        if sender_country:
            full_sender_address.append(sender_country)
        
        if full_sender_address:
            result["sender_full_address"] = "\n".join(full_sender_address)

    # ── Enhanced Recipient Address Extraction ──────────────────────────────
    # Extract complete recipient address block
    recipient_block = re.search(r'Recipient\s*\n(.*?)(?=Automation|Transportation|Charges)', t, re.DOTALL)
    if recipient_block:
        ...
        # Extract recipient address components
        recipient_address_parts = []
        recipient_city_state_zip = ""
        recipient_country = ""
        
        for i, line in enumerate(recipient_lines[1:], 1):
            # Check for city, state/province, postal code pattern
            city_state_zip_match = re.search(r'^(.+?),?\s+([A-Z]{2})\s+([A-Z0-9\s-]+)$', line)
            if city_state_zip_match:
                ...
                recipient_city_state_zip = line
                continue
            
            # Check for country (usually last line or after postal code)
            if line.upper() in ['CANADA', 'UNITED STATES', 'USA', 'US', 'CA']:
                result["recipient_country"] = line.upper()
                recipient_country = line
                continue
        ...
        # Create full recipient address
        full_recipient_address = []
        if "recipient_name" in result:
            full_recipient_address.append(result["recipient_name"])
        if recipient_address_parts:
            full_recipient_address.extend(recipient_address_parts)
        if recipient_city_state_zip:
            full_recipient_address.append(recipient_city_state_zip)
        if recipient_country:
            full_recipient_address.append(recipient_country)
        
        if full_recipient_address:
            result["recipient_full_address"] = "\n".join(full_recipient_address)
